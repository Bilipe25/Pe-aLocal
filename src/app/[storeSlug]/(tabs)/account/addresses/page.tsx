...
export default async function ConsumerAddressesPage({
  ...
  const token = (await cookies()).get(CONSUMER_SESSION_COOKIE)?.value;
  let addresses;
  try {
    addresses = await listConsumerAddresses({ storeSlug: store.slug, sessionToken: token });
  } catch {
    ...
    <div className="storefront-page-bottom-safe">
      ...
      <main className="storefront-orders-main">
        <p className="text-text-secondary mb-5 text-sm">
          O frete e a área de entrega serão recalculados no checkout.
        </p>
        <ConsumerAddresses storeSlug={store.slug} addresses={addresses} />
      </main>
    </div>
  );
}
