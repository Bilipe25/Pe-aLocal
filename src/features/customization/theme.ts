...
export function getStorefrontThemeStyle(config: StoreCustomizationConfig): StorefrontStyle {
  return {
    ...
    '--store-skeleton-base': `color-mix(in srgb, ${config.palette.text} 13%, ${config.palette.surface})`,
    '--store-skeleton-highlight': `color-mix(in srgb, ${config.palette.primary} 11%, ${config.palette.surface})`,
    '--store-skeleton-border': `color-mix(in srgb, ${config.palette.text} 14%, ${config.palette.border})`,
